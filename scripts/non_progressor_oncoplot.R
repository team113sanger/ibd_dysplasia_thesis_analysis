...
# Add missing samples
variants_unique_full <- add_missing_samples(non_progressors, variants_unique, "No mutation")
...
######## Prepare Metadata ########
meta <- metadata |>
  filter(sanger_dna_id %in% non_progressors) |>
  rename(
...
ggsave("plots/non_progressors_oncoplot.pdf",
  height = 5.5, width = 7
)